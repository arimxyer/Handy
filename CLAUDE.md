...
- `insiders` is merged through upstream `v0.9.5` (merge `036b8a9`, integrated on a throwaway `merge/upstream-v0.9.5` branch and folded in, 2026-08-09). `insiders-version.json` reads `0.9.5-insiders.3`, and that build is installed as of 2026-08-09. `.2` cherry-picked two upstream paste commits ahead of their release and `.3` added the fork's own dotool availability + fall-through fix; both were developed on `feat/upstream-paste-fixes` and fast-forwarded into `insiders` on 2026-08-09, so that branch no longer exists. Described below. The manifest is bumped per fork build precisely so two builds of the same upstream version stay distinguishable in the tray and in the backup inventory.
...
- Rollback material in `/home/arimayer/backups`, newest first:
  - `Handy_0.9.5-insiders.3_amd64.deb` — what is installed; reinstall from this.
