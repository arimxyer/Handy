...
- Rollback material in `/home/arimayer/backups`: `Handy_0.9.4-insiders.1_amd64.deb` (what is installed — reinstall from this), `handy-premerge-20260802` (settings + history.db as of the upgrade), `handy-insiders-0.8.3-insiders.1` (the previous binary). The May 2026 `handy-upgrade-*` snapshots were deleted on 2026-08-02: their profile copies held 150/170 history rows against 261 live, so they were strict subsets of current data. `/usr/lib/Handy` now holds 0.9.4's runtime libs, so rolling back past 0.9.4 means rebuilding the older tag from git rather than restoring a directory.
