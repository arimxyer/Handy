...
- `insiders` is merged through upstream `v0.9.5` (merge `036b8a9` on `merge/upstream-v0.9.5`, 2026-08-09). `insiders-version.json` reads `0.9.5-insiders.1`. The installed binary is still `0.9.4-insiders.1` — 0.9.5 has not been built or installed yet.
...
- v0.9.5 adds exactly one settings key, `selected_channel: Option<u16>` (input-channel selection for multi-channel interfaces), with `#[serde(default)]` and no `settings_schema_version` bump. Nothing rewrites the existing profile.
