...
- `insiders` is merged through upstream `v0.9.5` (merge `036b8a9` on `merge/upstream-v0.9.5`, 2026-08-09). `insiders-version.json` reads `0.9.5-insiders.3`, and that build is installed as of 2026-08-09. `.2` and `.3` are `insiders` plus branch `feat/upstream-paste-fixes`: `.2` cherry-picked two upstream paste commits ahead of their release, `.3` added the fork's own dotool availability + fall-through fix. Both described below. The manifest is bumped per fork build precisely so two builds of the same upstream version stay distinguishable in the tray and in the backup inventory.
